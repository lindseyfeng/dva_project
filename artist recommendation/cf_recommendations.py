...
class ArtistRecommender:
    ...
    def get_cluster_for_artist(self, artist_name):
        """
        ...
        cluster_info = df[df['artists'].str.contains(artist_name, case=False, regex=False)]
        return cluster_info['cluster'].iloc[0] if not cluster_info.empty else "Artist not found"
    ...
    def get_filtered_artist_data(self, artist_list):
        ...
        """
        return self.spotify_data_df[self.spotify_data_df[' "artistname"'].isin(artist_list)]
    ...
    def create_interaction_matrix(self, filtered_artist_df):
        ...
        """
        return pd.crosstab(filtered_artist_df['user_id'], filtered_artist_df[' "artistname"'])
    ...
    def recommend_artists(self, interaction_matrix, new_user_interactions, artist_names, top_n=5):
        """
...
def main():
    ...
    cluster_id = recommender.get_cluster_for_artist(favorite_artist)
    ...
    recommendations = recommender.recommend_artists(csr_matrix(cross_df.values), new_user_interactions, artist_names)
